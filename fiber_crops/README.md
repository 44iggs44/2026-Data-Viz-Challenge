...
#### 1. Open the Archive.zip file or 2. Run the ```text project_DataViz_fiber_team.R``` file
...
##### Option 2. Run the ```text project_DataViz_fiber_team.R``` file.
...
| | |___manufacturing
```
<small> *Note*: ```text textile_industry_output_import``` are not generated by running the ```text project_~.R``` file.
<!---> COMMENT
I'm still a bit unsure of where the data for these files came from, was it from the ERS_cotton.R file?
<!---> 
<\small>
